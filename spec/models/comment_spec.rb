  ...
  describe "#save" do
      context "with valid params" do
        it "should save user data" do
          ...
          insert_query = "INSERT INTO comment (user_id, post_id, body) VALUES ('#{comment_params["user_id"]}', '#{comment_params["post_id"]}', '#{comment_params["body"]}')"
          allow(Mysql2::Client).to receive(:new).and_return(comment_mock)
          new_comment = Comment.new(comment_params["user_id"], comment_params["post_id"], comment_params["body"], nil)
